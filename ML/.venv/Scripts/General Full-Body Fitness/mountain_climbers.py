...
class PoseDetector:
    ...
    def findAngle(self, img, p1, p2, p3,joint_type=None, draw=True):
        ...
        # Adjust angle based on joint type (if needed)
        if joint_type == 'shoulder':
            # Shoulder angle is typically between 90° and 150°
            pass
        elif joint_type == 'elbow':
            # Elbow angle is typically between 90° and 180°
            pass
        elif joint_type == 'hip':
            # Hip angle is typically between 90° and 150°
            pass
        elif joint_type == 'knee':
            # Knee angle is typically between 90° and 180°
            pass
        elif joint_type == 'ankle':
        # Ankle angle is typically between 90° and 120°
            pass
...
def main():
    ...
    while cap.isOpened():
        ...
        if len(lmList) != 0:
            ...
            combined_angle = (0.25 * elbow) + (0.2 * shoulder) + (0.25 * hip) + (0.2 * knee) + (0.1 * ankle)
            # Add level checks for form (beginner, intermediate, expert)
            if elbow >= 120 and elbow <= 150 and shoulder >= 90 and shoulder <= 120 and hip >= 120 and hip <= 150 and knee >= 150 and knee <= 180 and ankle >= 90 and ankle <= 120:
                form = "Beginner"
            elif elbow >= 90 and elbow < 120 and shoulder >= 60 and shoulder < 90 and hip >= 90 and hip < 120 and knee >= 120 and knee < 150 and ankle >= 60 and ankle < 90:
                form = "Intermediate"
            elif elbow >= 60 and elbow < 90 and shoulder >= 30 and shoulder < 60 and hip >= 60 and hip < 90 and knee >= 90 and knee < 120 and ankle >= 30 and ankle < 60:
                form = "Expert"
            else:
                form = "Incorrect"
           ...
           # Check for proper push-up position and provide detailed feedback
            if form == "Beginner" or form == "Intermediate" or form == "Expert":
                if bar <= 50:  # Check if the progress bar is at the top (Up position)
                    if direction == 0:
                        feedback = "Knee Forward"
                      # Knee backward position
                        direction = 1
                elif bar >= 380:  # Check if the progress bar is at the bottom (Down position)
                    if direction == 1:
                        feedback = "Knee Backward"
                        count += 1 # Count one full repetition
                        direction = 0
            else:
                feedback = "Fix Form"
            ...
            if form == "Beginner":
                feedback = "Beginner Form: Try to drive knees higher. Angles: Elbow 120°-150°, Shoulder 90°-120°, Hip 120°-150°, Knee 150°-180°, Ankle 90°-120°."
            elif form == "Intermediate":
                feedback = "Intermediate Form: Improve by driving knees closer to chest. Angles: Elbow 90°-120°, Shoulder 60°-90°, Hip 90°-120°, Knee 120°-150°, Ankle 60°-90°."
            elif form == "Expert":
                feedback = "Expert Form: Excellent Mountain Climber posture! Angles: Elbow 60°-90°, Shoulder 30°-60°, Hip 60°-90°, Knee 90°-120°, Ankle 30°-60°."
            else:
        ...
        cv2.imshow('Mountain climber Counter', img)
