    ...
    while cap.isOpened():
        ...
        if len(lmList) != 0:
            ...
            if left_shoulder_x < right_shoulder_x:
                ...
                spine = detector.findAngle(img, 11, 23, 25)  # Spine angle (shoulder-hip-knee)
                ...
                spine = detector.findAngle(img, 12, 24, 26)  # Spine angle (shoulder-hip-knee)left_shoulder = detector.findAngle(img, 14, 12, 24)  # Right Shoulder angle (now considered left)
            ...
            combined_angle = (0.5 * spine) + (0.3 * hip) + (0.2 * shoulder)  # Weighted average
            # Add level checks for form (beginner, intermediate, expert)
            if spine >= 120 and spine <= 150 and hip >= 120 and hip <= 150 and shoulder >= 120 and shoulder <= 150:
                form = "Beginner"
            elif spine >= 150 and spine <= 170 and hip >= 150 and hip <= 170 and shoulder >= 150 and shoulder <= 170:
                form = "Intermediate"
            elif spine >= 170 and spine <= 180 and hip >= 170 and hip <= 180 and shoulder >= 170 and shoulder <= 180:
                form = "Expert"
            else:
                form = "Incorrect"
           ...
           # Check for proper push-up position and provide detailed feedback
            ...
            if spine < 120:
                feedback = "Spine not extended enough! Arch your back more in Cow Pose."
            if hip < 120:
                feedback = "Hips not engaged! Drop your belly towards the floor in Cow Pose."
            if shoulder < 120:
                feedback = "Shoulders not aligned! Keep shoulders over wrists."
            # Categorizing catcow- form by angle levels
            if form == "Beginner":
        ...
        cv2.imshow('Cat Cow Stretch Counter', img)
